...
Primero hemos creado una función que obtiene la url absoluta independientemente del input.
...
Hemos añadido las url absolutas a una nueva columna del dataframe.

Con la url absoluta hemos usado la función HEAD para determinar el status del enlace y lo hemos guardado en una columna del dataframe.

```{r}
df_enlaces_dedup$status <- NA
...
Hemos creado una nueva columna en el dataframe que indica si la url original era absoluta o no. A partir de esta columna i la de veces que aparece cada enlace hemos generado un histograma.

El cian muestra el numero de urls absolutas y el naranja muestra el número de urls relativas.  
```{r}
...
ggplot(df_enlaces_dedup, aes(x = n, fill = is_absolute)) + geom_histogram(alpha = 1, position = "dodge", bins = 2) + scale_fill_manual(values = c("TRUE" = "#0092EB", "FALSE" = "#EB8900")) + theme_minimal() + scale_x_continuous(breaks=c(1,2))
...
En esta sección, añadimos una nueva columna en el dataframe que indica si la url redirije al propio dominio o no. 
...
ggplot(df_enlaces_dedup, aes(x = in_domain)) +
  geom_bar(aes(fill = in_domain)) +
  scale_fill_manual(values = c("TRUE" = "#EBE70B", "FALSE" = "#6900EB")) +
  theme_minimal()
...
Primeramente hacemos un conteo de el número de cada status code y luego hacemos un piechart con ello. 

En nuestro caso, todos los códigos són `200`. 

```{r}
df_status <- df_enlaces_dedup %>% count(status)

ggplot(df_status, aes(x = "", y = n, fill = factor(status))) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  theme_void() +
  scale_fill_brewer(palette = "Set3") +
  labs(fill = "Status HTTP",
       title = "Distribució de codis HTTP")
